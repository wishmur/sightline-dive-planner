...
**One flip crosses the adoption bar.** The verifier is right on 46 of 78 claims (59%) against a
pre-registered bar of 60%, which needs 47. All 25 claims below are ones it called "partial" and
the gold calls "supported", so each claim adjudicated in the verifier's favour is worth exactly
one point: 1 flip reaches 60.3%, 5 reach 65%, all 25 reach 91%. This packet therefore decides
whether the verifier is adopted, on a margin of a single claim.

That is precisely the situation in which a number should not be watched while it is being made.
The protocol, fixed before any decision:

1. Judge each claim **only against the quoted cited pages**, not general knowledge, and not
   against whether the verifier "seems" too strict. The question is narrow: does the cited page
   state the specific detail the verifier says is missing?
2. Decide **all 25** before computing anything. `bun evals/adjudicate.ts --score` withholds the
   post-adjudication accuracy until the packet is complete, on purpose.
3. A flip or an unclear needs a reason on the **Why** line; the tool rejects the packet without one.
4. The published 59% stands. `--apply` records the adjudication beside the original verdict and
   never rewrites it, so the pre-adjudication number keeps replaying to exactly what was reported.
   The post-adjudication number is published as its own figure, with the adjudicator and the date.
5. If the outcome is that the gold was lenient, the fix is a **stated strictness rule** for future
   labelling, not a quiet relabelling of this set.

## Workflow

```
# mark one box per claim below, add a Why for any flip or unclear
bun evals/adjudicate.ts                          # progress and validation
bun evals/adjudicate.ts --apply --by "NAME"      # record the decisions, dated
bun evals/adjudicate.ts --score                  # pre- and post-adjudication, side by side
```
